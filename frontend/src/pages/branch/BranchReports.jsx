...
export default function BranchReports() {
  ...
  const fetchTransactions = useCallback(() => {
    setTrxLoading(true);
    // branch_id otomatis dari JWT di backend — tidak perlu kirim manual
    getTransactions({ page: trxPage, limit: 20, date_from: dateFrom, date_to: dateTo })
      .then(r => { setTransactions(r.data?.data?.data || []); setTrxMeta(r.data?.data || {}); })
      .catch(console.error)
      .finally(() => setTrxLoading(false));
  }, [trxPage, dateFrom, dateTo]);
  ...
  useEffect(() => { setTrxPage(1); }, [dateFrom, dateTo]);

  const handleExportExcel = async () => {
    setExporting(true);
    try {
      if (tab === "transactions") {
        const res = await getTransactions({ limit: 9999, date_from: dateFrom, date_to: dateTo });
        exportTransactionsExcel(res.data?.data?.data || [], `laporan-cabang-${dateFrom}-${dateTo}`);
      } else {
        const res = await getProducts({ limit: 9999 });
        exportProductsExcel(res.data?.data?.data || [], "laporan-stok-cabang");
      }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const res = await getTransactions({ limit: 9999, date_from: dateFrom, date_to: dateTo });
      exportTransactionsPDF(res.data?.data?.data || [], `Laporan Cabang ${dateFrom} s/d ${dateTo}`, `laporan-cabang-${dateFrom}-${dateTo}`);
  ...
  const totalRevenue = transactions.reduce((s, t) => s + parseFloat(t.grand_total), 0);
  const totalItems   = transactions.reduce((s, t) => s + (t.items?.length || 0), 0);

  return (
    <AppLayout title="Laporan Cabang" subtitle="Analisis penjualan dan stok cabang Anda">
      {/* Tabs */}
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5">
            {[
              ...
              { label: "Total Pendapatan",  value: fmt(totalRevenue),   color: "text-emerald-600" },
              { label: "Total Item Terjual", value: totalItems,         color: "text-amber-600" },
            ].map(s => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    {["Kode", "Waktu", "Kasir", "Item", "Total", "Metode", "Status"].map(h => (
                  ...
                  {trxLoading ? (
                    Array(5).fill(0).map((_, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0">
                        {Array(7).fill(0).map((_, j) => <td key={j} className="px-5 py-4"><Skeleton height={14} /></td>)}
                      </tr>
                    ))
                  ) : transactions.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-12 text-center text-sm text-slate-400">Tidak ada transaksi pada periode ini</td></tr>
                  ) : transactions.map(t => (
          ...
          <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-100 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    {["Produk", "SKU", "Kategori", "Harga Jual", "Stok", "Min. Stok", "Status Stok"].map(h => (
                      <th key={h} className="px-5 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {prodLoading ? (
                    ...
                    <tr><td colSpan={7} className="px-5 py-12 text-center text-sm text-slate-400">Belum ada produk</td></tr>
                  ) : products.map(p => {
                    const s = p.stocks?.[0];
                    return (
                      <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="px-5 py-3.5 font-medium text-slate-800">{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(p.selling_price)}</td>
